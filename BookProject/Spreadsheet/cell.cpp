...
/*!
 * \brief Cell::Cell set the cashe is dirty.
 */
Cell::Cell()
{
    setDirty();
...
/*!
 * \brief Cell::clone This function is called when there is a new Cell to be
 * created.
 * \return
 */
...
QVariant Cell::value() const
{
    if (casheIsDirty) {
        ...
        if (formularStr.startsWith('\'')) {
            ...
            bool ok;
            double d = formularStr.toDouble(&ok);
            if (ok) {
                cachedValue = d;
            } else {
