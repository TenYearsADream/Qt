...
{
    int secs = static_cast<int>(QDateTime::currentDateTime().secsTo(finishTime));
    if (secs < 0) {
        secs = 0;
    }
    return secs;
...
void OvenTimer::mousePressEvent(QMouseEvent* event)
{
    ...
    setDuration(duration() + static_cast<int>(theta / DegreesPerSecond));
    update();
}
