...
MainTopWidget::MainTopWidget(QWidget *parent) : BaseStyleWidget(parent) {
  this->initUI();
  this->initAnimations();
  this->initConnect();
}
...
void MainTopWidget::initAnimations() {
  ...
  QPropertyAnimation *mainExamineAnim =
  ...
  QPropertyAnimation *scoreExamineAnim =
      new QPropertyAnimation(m_scoreWidget, "pos");
  ...
  QPropertyAnimation *scoreReturnAnim =
      new QPropertyAnimation(m_scoreWidget, "pos");
...
void MainTopWidget::initUI() {
  ...
  m_scoreWidget->setTextInfo(QStringLiteral("1234"),
                             QStringLiteral("1234"));
...
void MainTopWidget::initTopTitleWidget() {
  ...
  textLabel->setText(QStringLiteral("360 10.0 Beta"));
...
void MainTopWidget::initConnect() {
  connect(m_bottomWidget, SIGNAL(examineClicked()), this,
          SLOT(examineClicked()));
  connect(m_bottomWidget, SIGNAL(viewClicked()), this, SLOT(viewClicked()));
  connect(m_returnGroupAnimation, SIGNAL(finished()), this,
          SLOT(returnAnimationFinished()));
}

void MainTopWidget::setNums(int num) { m_backgroundWidget->setNums(num); }
